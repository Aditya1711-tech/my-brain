...
@router.get("/threads")
async def list_threads(db: DbSession, user_id: VerifiedUser) -> list[dict]:
    """List chat threads for a user."""
    repo = ChatRepo(db)
    return await repo.list_threads(user_id)


@router.get("/threads/{thread_id}/messages")
async def get_thread_messages(
    thread_id: str, db: DbSession, user_id: VerifiedUser,
) -> list[dict]:
...
async def delete_thread(
    ...
    repo = ChatRepo(db)
    deleted = await repo.delete_thread(thread_id, user_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Thread not found")
    await db.commit()
    return {"deleted": True}
